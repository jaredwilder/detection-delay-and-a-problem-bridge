...
def check_reciprocal_heat_pde():
    z=sp.symbols("z")
    E=sp.Function("E")(z)
    # H = 1/E.  H_t = -4 z H'' - 2 H'; E_t=-H_t/H^2.
    H=1/E
...
def check_dual_jt_schur_numeric():
    # G = product(1+x_j z), so a_n=e_n(x).
    xs=[sp.Rational(1,2),sp.Rational(1,3),sp.Rational(1,5),
        sp.Rational(1,7),sp.Rational(1,11)]
    a=elementary(xs)
    # verify a few rectangular determinants are positive
    for r in range(1,4):
        for k in range(1,4):
            val=sp.simplify(det_from_coeff(a,r,k))
            assert val>0

def check_phase_budget_example():
    # One conjugate pair plus positive real variables.
    theta=0.08
    rho=0.2
    xs=[rho*cmath.exp(1j*theta),rho*cmath.exp(-1j*theta),
        0.12,0.09,0.05,0.03]
    # elementary coefficients numerically
    a=[1+0j]
    for x in xs:
        a.append(0j)
...
def check_occupancy_cap_bruteforce():
    # Enumerate SSYT for a 2-row x 3-column rectangle: k=2 rows, r=3 columns.
    # Columns strictly increasing, rows weakly increasing.
    ...
    assert count>0
    assert max(max_occ)<=cols  # r = number of columns
...
def main():
    ...
    print("NOTE: These checks do not prove RH or Terminal Candidate V-B.")
